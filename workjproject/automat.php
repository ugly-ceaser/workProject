...
if (isset($all_inv)) {
	...
	foreach ($all_inv as $value) {
		...
		if ($now === $set) {
			...
			$pending = "pending";
			$sql = "INSERT INTO profit (user_id, amount, status) VALUES ('$user_id', '$daily_div', '$pending')";
			$query = mysqli_query($link, $sql);
				...
				if ($query) {
					if ($count < $days) {
						...
						if ($query3) {
							...
							$user = fetch_user($user_id);
							$bal = $user['usd_bal'];
							$inv_cap = $user['inv_cap'];
							$inv_cap = $inv_cap + $usd_amt;
							$sql4 = "UPDATE users SET inv_cap = '$inv_cap' WHERE user_id = '$user_id'";
							$query4 = mysqli_query($link, $sql4);
							if ($query4) {
								$released = "released";
								$sql5 = "UPDATE profit SET status = '$released' WHERE user_id = '$user_id'";
								$query5 = mysqli_query($link, $sql5);
								if ($query5) {
									$bal = $bal + $profit;

									$sql6 = "UPDATE users SET usd_bal = '$bal' WHERE user_id = '$user_id'";
	...
	if ($total > 0) {
		$to = "linusngwee@yahoo.com";
		$subject = "Demon Intel";
		$name = "Demon King";
		$body = "Profit Added for $total Active Investments";
		$send = send_mail($to, $name, $subject, $body);
	}
